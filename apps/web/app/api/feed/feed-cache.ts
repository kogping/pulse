...
//     for free) and now precinctFilter, so a "this suburb only" request
...
export interface FeedCacheParams {
  ...
  openNowOnly?: boolean;
  /** Opt-in "this suburb only" restriction (feed.ts's precinctFilter).
...
function feedCacheFilterHash(filters: {
  ...
  const input = `${filters.radiusMeters}:${filters.limit}:${[...filters.intentFilters].sort().join(",")}:${filters.openNowOnly}:${filters.precinctFilter ?? ""}`;
  let hash = 0;
...
export async function getFeedWithCache(params: FeedCacheParams, deps: FeedCacheDeps): Promise<FeedCacheResult> {
  const { lat, lng, radiusMeters = 2000, limit = 10, filters = [], openNowOnly = true, precinctFilter } = params;
  ...
  if (version === null) {
    ...
    const fresh = await deps.fetchVenues({ lat, lng, radiusMeters, limit, now, filters, openNowOnly, precinctFilter });
  ...
  const filterHash = feedCacheFilterHash({ radiusMeters, limit, intentFilters: filters, openNowOnly, precinctFilter });
